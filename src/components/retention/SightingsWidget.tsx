    ...
    <div className="bg-gradient-to-br from-arctic-800/50 to-arctic-900/50 rounded-xl p-6 border border-white/10">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-semibold">{t('liveObservations')}</h3>
            <p className="text-white/60 text-sm">{t('last30Minutes')}</p>
          </div>
        </div>
        <div className="text-right">
          <div className="text-3xl font-bold text-primary">{recentSightings}</div>
          <div className="text-xs text-white/60">{t('reports')}</div>
        </div>
      </div>
        ...
        {recentSightings > 5 && (
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-green-500/10 border border-green-500/20">
            ...
            <p className="text-sm text-green-400 font-medium">
              {t('highActivityCount').replace('{count}', String(recentSightings))}
            </p>
          </div>
        )}
